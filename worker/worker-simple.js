export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/sync") {
      ...
      return new Response('{"status":"started"}', {headers:headers});
    }
    return new Response('{"endpoints":["/sync"]}', {headers:headers});
  }
};
...
async function runSync(db, oddsKey) {
  ...
  var d2 = today.toISOString().split("T")[0];
  var d3 = new Date(today.getTime() + 86400000).toISOString().split("T")[0];
  var dates = [d1, d2, d3];
